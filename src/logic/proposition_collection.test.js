...
describe('BARBARA tests', () => {
  ...
  test('BARBARA reduces correctly', () => {
    const { reducedTableUnified } = argument.reduce([c, b]);
    ...
    expect(JSON.stringify(reducedTableUnified)).toBe(expectedReducedTableUnified);
  });
...
describe('Sorites test', () => {
  ...
  test('Sorites reduces correctly', () => {
    const { reducedTableUnified } = argument.reduce([c, d]);
